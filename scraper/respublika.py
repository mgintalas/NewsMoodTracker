...
def scrape_website(url, source):
    try:
        ...
        titles = soup.find_all('h2', class_='news_title_red')
        ...
        for title in titles:
            ...
            # Regular expression to remove the time and leading whitespace
            original_title = re.sub(r'^[\w\.]+\.\w+\s+\d+\s+min\s+', '', original_title)
            ...
            try:    
                article_link_find = title.find('a')['href']
                article_link = URL + article_link_find
            except TypeError:
                article_link = 'Link not available'
